import { GameNumber } from "@prisma/client";
...
export async function generateNewNumber(gameId: number) {
  ...
  if (nextNumber === null) throw new Error("No number available");
...
function checkIfAllNumbersAlreadyBeenDrawn(numbers: GameNumber[]) {
...
function extractOnlyTheValues(numbers: GameNumber[]) {
